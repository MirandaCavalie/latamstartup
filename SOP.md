...
Rebranding a La Combi (17/09/2026): el usuario sustituyó la chancla por un colectivo y pidió una identidad inspirada en cartelería, collage y azulejos populares latinoamericanos. La portada usa un PNG original de 1448 × 1086, generado con imagegen integrado, con el texto exacto “Tu envidia es mi progreso”: papel amarillo, letras rojas y azules, flores y hojas de borde. No es el logo. Entrada, encabezado, pie y favicon usan `combi-mark.png` (1254 × 1254 con transparencia real) y el nombre La Combi. La interfaz combina papel crema, amarillo, rojo, cobalto y verde; el globo mantiene su geometría e interacción, con país seleccionado en amarillo/azul. Banderas y logos oficiales conservan sus colores y archivos. Los prompts exactos y la procedencia están en `public/brand/COMBI-PROMPTS.md`.

Presentación del cartel: inclinación de −2°, sombra tenue y entrada de 850 ms, una sola vez. Al pasar el cursor se endereza y levanta ligeramente. La combi llega en 950 ms, la marca en 700 ms, la etiqueta en 700 ms y el trazo del globo en 1600 ms; ninguno usa bucle permanente. El logo responde al cursor durante 420 ms. `prefers-reduced-motion: reduce` desactiva las animaciones y transiciones. El texto alternativo conserva el encabezado accesible. No se aplican filtros de color a las imágenes.
...
- Entrada visual: PNG del cartel y marca de combi en `public/brand/`, logos oficiales PNG/SVG/WebP en `public/logos/`. Las imágenes no son enlaces a servicios de logos ni dependen de un proveedor externo durante la visita.
...
- Mantener el cartel separado del símbolo de identidad. Para cambiar su texto, regenerar el asset y actualizar `components/chicha-poster.tsx`, dimensiones, texto alternativo y pruebas; no reconstruirlo con una fuente CSS. Conservar colores sin filtros y respetar movimiento reducido. Los prompts actuales están en `public/brand/COMBI-PROMPTS.md`; los tokens de color están en `app/globals.css`. La marca y los títulos usan Bowlby One SC, alojada localmente en `public/fonts/` con licencia OFL y fuentes registradas; está inspirada en la referencia Sabor LATAM, sin afirmar que sea la misma fuente. El contenido y los controles conservan Geist para legibilidad.
